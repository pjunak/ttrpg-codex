...
export async function exercisePlannerLive({ t, open, admin, csrf }) {
  ...
  await page.waitForFunction(() => document.querySelector('form[aria-label="Planning item details"] input[name="title"]')?.value === 'Updated in another tab');
  ...
  assert.equal(await zoom.textContent(), zoomBefore);

  await title.fill('Keep my unfinished title'); await title.evaluate(input => input.setSelectionRange(5, 9));
  ...
  assert.deepEqual(await title.evaluate(input => [document.activeElement === input, input.selectionStart, input.selectionEnd]), [true, 5, 9]);
  ...
  assert.equal((await records()).find(record => record.key === item.id).value.title, 'Another remote title');
