...
export async function exerciseCzechSheet({ page, key, mobile, output, get, write }) {
  ...
  assert.equal(inventory.find(item => item.ref === 'longsword').snapshot.name, 'Longsword');
  assert.equal(inventory.find(item => item.name === 'Klíč River').location, 'pack');
  assert.equal(inventory.find(item => item.id === 'rope').location, 'ready');
  ...
  assert.equal(copied.currency.gp, 50); assert.equal(copied.inventory.find(item => item.id === 'scroll').qty, 1);
