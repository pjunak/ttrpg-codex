...
export async function exercisePlannerAnnotations({ t, open, admin, csrf, output, mobile }: Pick<InstalledFixture, 't' | 'open' | 'admin' | 'csrf' | 'output' | 'mobile'>) {
  ...
  const write = async (dataId: string, value: Record<string, unknown>, expectedRevision = 0) => jsonResponse(await admin.post(`${base}/transactions`, { headers,
  ...
  let reference = (await records('planning_references')).find((record: FixtureRecord) => record.value.itemId === id);
  ...
  reference = (await records('planning_references')).find((record: FixtureRecord) => record.key === reference.key);
  ...
  let savedConsequence = (await records('planning_consequences')).find((record: FixtureRecord<{ anchor: { itemId: string } }>) => record.value.anchor.itemId === id);
  ...
  await consequence.getByRole('button', { name: 'Save consequence', exact: true }).click(); await page.getByText('Consequence saved.', { exact: true }).waitFor();
  savedConsequence = (await records('planning_consequences')).find((record: FixtureRecord) => record.key === savedConsequence.key);
  ...
  let savedNote = (await records('dm_notes')).find((record: FixtureRecord<{ anchorIds: string[] }>) => record.value.anchorIds.includes(id));
  ...
  savedNote = (await records('dm_notes')).find((record: FixtureRecord) => record.key === savedNote.key); assert.deepEqual(savedNote.value.anchorIds, [peer]);
