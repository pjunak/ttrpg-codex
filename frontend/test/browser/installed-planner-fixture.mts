...
export async function exercisePlannerEditing({ t, open, admin, csrf, output }: Pick<InstalledFixture, 't' | 'open' | 'admin' | 'csrf' | 'output'>) {
  ...
  const transaction = async (mutation: unknown) => jsonResponse(await admin.post(`${base}/transactions`, { headers,
  ...
  let event = (await records('planning_items')).find((record: FixtureRecord) => record.value.title === 'Editor encounter');
  assert.ok(event);
  ...
  const branch = (await records('planning_items')).find((record: FixtureRecord) => record.value.title === 'Editor choice');
 ...
 const { promise: held, resolve: releaseWrite } = Promise.withResolvers<void>();
 const { promise: received, resolve: writeReceived } = Promise.withResolvers<void>();
  ...
  event = (await records('planning_items')).find((record: FixtureRecord) => record.key === eventId);
  ...
  assert.equal((await records('planning_items')).find((record: FixtureRecord) => record.key === eventId).value.title, 'Remote saved title');
  ...
  const note = (await records('dm_notes')).find((record: FixtureRecord<{ anchorIds: string[] }>) => record.value.anchorIds.includes(eventId));
  ...
  await eventCard.scrollIntoViewIfNeeded(); const position = await eventCard.boundingBox().then(required);
