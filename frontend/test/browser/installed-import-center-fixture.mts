...
import { importPackageFiles, planningImport, importQuest } from './installed-import-fixture.mts';
...
export async function exerciseImportCenter({ t, root, output, open, admin, csrf, disable, mobile }: InstalledFixture & {
  root: string; disable: (id: string) => Promise<unknown>;
}) {
  const archive = await readFile(resolve(required(process.env.CODEX_DM_TOOLS_ZIP)));
  const installed = new Set<string>(); let consumer = false;
  t.after(async () => { if (consumer) await disable('dm-tools'); for (const id of [...installed].reverse()) await disable(id); });
  const installProvider = async (id: string) => { await installReviewedPackage(admin, csrf, id, await providerPackage({ id, root, output, archive }), []); installed.add(id); };
  ...
  const choose = (value: unknown, name = 'notes.json') => file.setInputFiles({ name, mimeType: 'application/json', buffer: Buffer.from(typeof value === 'string' ? value : JSON.stringify(value)) });
  ...
  const sent: { method: string; [key: string]: unknown }[] = []; page.on('request', request => { if (request.url().endsWith('/services/call')) sent.push(request.postDataJSON()); });
  for (const [source, error] of [['invalid', 'not valid JSON'], ['[]', 'top-level'], [{ format: 'unsupported' }, 'No enabled importer']] as const) {
  ...
  assert.equal((await records()).some((record: FixtureRecord) => record.key === id), false);
  const { promise: held, resolve: release } = Promise.withResolvers<void>(), { promise: pending, resolve: started } = Promise.withResolvers<void>(); t.after(() => release());
  ...
  assert.equal((await records()).find((record: FixtureRecord) => record.key === id).value.text, text);
  ...
  const { promise: heldPreview, resolve: releasePreview } = Promise.withResolvers<void>(), { promise: pendingPreview, resolve: previewStarted } = Promise.withResolvers<void>(); t.after(() => releasePreview());
  ...
  assert.equal(await page.locator('.dm-import-preview').count(), 0); assert.equal((await records()).some((record: FixtureRecord) => record.key.endsWith('-cancel')), false);
