...
export async function exercisePlannerFlows({ t, open, admin, csrf, output }: Pick<InstalledFixture, 't' | 'open' | 'admin' | 'csrf' | 'output'>) {
  ...
  const transact = async (mutations: unknown[]) => jsonResponse(await admin.post(`${base}/transactions`, { headers,
  ...
  const item = (id: string, title: string, kind: string, parentId: string | null = null) => ({ id, title, kind, parentId, schemaVersion: 3, summary: '', objective: '', body: '', setup: '', resolution: '', tags: [], updatedAt: 1,
  ...
  const storedFlow = (await records('planning_flow_links')).find((record: FixtureRecord) => record.value.sourceId === 'flow-branch');
  ...
  const card = (id: string) => page.locator(`.dm-plan-card[data-item-id="${id}"]`);
  ...
  assert.equal((await records('planning_flow_links')).find((record: FixtureRecord) => record.key === flowId).value.kind, 'continues');
  const label = page.locator('.dm-planner-flow-label').filter({ hasText: 'Enter <carefully>' }); await label.waitFor();
  assert.equal(await label.textContent().then(required), 'Enter <carefully>'); assert.equal(await label.locator('*').count(), 0);
  ...
  const consequence = (await records('planning_consequences')).find((record: FixtureRecord) => record.value.title === 'Planned consequence');
  ...
  assert.deepEqual((await records('planning_consequences')).find((record: FixtureRecord) => record.key === consequence.key).value.anchor, { scope: 'flow', flowId });
  ...
  let currentFlow = (await records('planning_flow_links')).find((record: FixtureRecord) => record.key === flowId);
  ...
  assert.equal((await records('planning_flow_links')).find((record: FixtureRecord) => record.key === flowId).value.label, 'Remote flow label');
  ...
  assert.ok((await records('planning_flow_links')).some((record: FixtureRecord) => record.key === flowId));
  const currentConsequence = (await records('planning_consequences')).find((record: FixtureRecord) => record.key === consequence.key);
  ...
  await page.getByText(/Reload the planner before making another change/u).waitFor();
  assert.ok((await records('planning_flow_links')).some((record: FixtureRecord) => record.key === flowId));
  assert.equal((await records('planning_consequences')).find((record: FixtureRecord) => record.key === consequence.key).value.body, 'Changed concurrently.');
  await page.getByRole('button', { name: 'Reload planner', exact: true }).click(); await page.locator('.dm-planner-shell[aria-busy="false"]').waitFor();
  ...
  assert.deepEqual(response.request().postDataJSON().mutations.map((mutation: FixtureMutation) => [mutation.dataId, mutation.key]), [['planning_flow_links', flowId], ['planning_consequences', consequence.key]]);
  await page.getByText('Flow removed.', { exact: true }).waitFor();
  assert.equal((await records('planning_consequences')).some((record: FixtureRecord) => record.key === consequence.key), false);
  assert.ok((await records('planning_consequences')).some((record: FixtureRecord) => record.key === 'flow-whole-item'));
  assert.ok((await records('planning_items')).some((record: FixtureRecord) => record.key === 'flow-event'));

  // Deleting the endpoint clears incoming references and only its shared-note anchor.
  const rootView = (await records('planning_views')).find((record: FixtureRecord) => record.key === 'scope-root');
  ...
  assert.equal((await records('planning_references')).some((record: FixtureRecord) => record.key === 'flow-incoming-ref'), false);
  assert.deepEqual((await records('dm_notes')).find((record: FixtureRecord) => record.key === 'flow-shared-note').value.anchorIds, ['flow-quest']);
  const positions = (await records('planning_views')).find((record: FixtureRecord) => record.key === 'scope-root').value.positions;
