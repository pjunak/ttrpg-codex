import type { GraphElement } from './fixture-dom.d.mts';
import { required, objectValue, fixtureCollection, fixtureRecord } from './fixture-types.mts';
...
const streams = new Set<import("node:http").ServerResponse>(), output = fileURLToPath(new URL('../../test-results/relationship-graph/', import.meta.url));
const collection = (name: string) => fixtureCollection(campaign, name);
const node = (page: Page, key: string) => page.locator(`.cm-node[data-key=${JSON.stringify(key)}]`);
const typedKey = (kind: string, key: string) => JSON.stringify([kind, key]);
const typedNode = (page: Page, kind: string, key: string) => node(page, typedKey(kind, key));
...
async function fixture(t: TestContext, { mobile = false, role = 'player', locale = 'en', blockedStorage = false, malformedStorage = false, mixed = false, reducedMotion = 'reduce' as 'reduce' | 'no-preference' } = {}) {
  ...
  fixtureRecord(collection('characters'), 'talia').value.faction = 'watch';
  ...
  if (mixed) {
    for (const record of collection('characters').records) {
      if (['ryn', 'mira'].includes(record.key)) { objectValue(record.value).faction = 'watch'; objectValue(record.value).location = 'gate'; }
      if (record.key === 'mira') objectValue(record.value).locationRoles = [{ locationId: 'outpost' }, { locationId: 'gate' }];
      if (record.key === 'talia') { objectValue(record.value).faction = 'guild'; objectValue(record.value).location = 'gate'; }
    }
...
async function positions(page: Page) { return page.evaluate(() => JSON.parse(localStorage.getItem('cm_pos_vztahy')!)); }
...
async function drag(page: Page, key: string, dx: number, dy: number) {
  const rect = await node(page, key).boundingBox().then(required); const x = rect.x + rect.width / 2, y = rect.y + 20;
...
for (const mobile of [false, true]) test(`relationship graph preserves card styling and zoom geometry (${mobile ? 'phone' : 'desktop'})`, async t => {
  ...
  const native = await node(page, 'ryn').evaluate(element => ({ width: (element as HTMLElement).offsetWidth, font: getComputedStyle(element.querySelector('.cm-name')!).fontSize,
  ...
  const shell = await page.locator('.cm-shell').boundingBox().then(required); assert.equal(Math.round(shell.x), mobile ? 0 : 240); assert.equal(Math.round(shell.height), mobile ? 784 : 1000);
  ...
  const zoomed = await node(page, 'ryn').evaluate(element => ({ width: (element as HTMLElement).offsetWidth, font: getComputedStyle(element.querySelector('.cm-name')!).fontSize,
  ...
  assert.equal(await page.getByRole('button', { name: 'Reset zoom to 100%', exact: true }).textContent().then(required), '100%');
});
...
test('pointer and keyboard moves persist legacy centers and never modify campaign records', async t => {
  ...
  assert.deepEqual(await page.locator('codex-campaign-graph').evaluate(graph => (graph as GraphElement).positions.get('ryn')!), { x: -195, y: -85 });
});
...
test('click, keyboard and context actions navigate to exact character details', async t => {
  ...
  assert.match(await node(page, 'talia').locator('.cm-cloud').getAttribute('class').then(required), /cm-vfilter-dim/);
...
test('chip filters, relationship toggles, focus depth and faction hiding survive reload', async t => {
  ...
  assert.match(await node(page, 'mira').locator('.cm-cloud').getAttribute('class').then(required), /cm-vfilter-dim/);
  ...
  assert.match(await node(page, 'kael').locator('.cm-cloud').getAttribute('class').then(required), /cm-vfilter-dim/);
  await page.getByRole('slider', { name: 'Neighborhood depth' }).fill('2'); assert.doesNotMatch(await node(page, 'kael').locator('.cm-cloud').getAttribute('class').then(required), /cm-vfilter-dim/);
...
test('cancelled drags and live changes preserve stored positions and remove unavailable endpoints', async t => {
  const { page } = await fixture(t); const rect = await node(page, 'ryn').boundingBox().then(required);
...
test('wheel zoom uses fixed steps and canvas panning does not change saved centers', async t => {
  ...
  const rect = await canvas.boundingBox().then(required); await page.mouse.move(rect.x + 20, rect.y + 20);
  ...
  const before = await page.locator('codex-campaign-graph').evaluate(graph => (graph as GraphElement).pan);
  await canvas.focus(); await page.keyboard.press('ArrowLeft');
  assert.equal(await page.locator('codex-campaign-graph').evaluate(graph => (graph as GraphElement).pan.x), before.x + 30);
...
test('long authored edge labels wrap completely along the connection', async t => {
  ...
  objectValue(collection('relationships').records[2].value).label = label;
  ...
  assert.ok(await text.locator('tspan').count() > 1); assert.doesNotMatch(await text.getAttribute('transform').then(required), /rotate\(0 /);
});
...
test('storage failures keep the local arrangement for retry and malformed preferences remain usable', async t => {
  ...
  const before = await page.locator('codex-campaign-graph').evaluate(graph => (graph as GraphElement).positions.get('ryn')!);
  await node(page, 'ryn').focus(); await page.keyboard.press('ArrowRight'); await page.locator('.cm-message[role="alert"]').waitFor();
  assert.equal(await page.locator('codex-campaign-graph').evaluate(graph => (graph as GraphElement).positions.get('ryn')!.x), before.x + 5);
...
test('another tab interrupts an active drag without overwriting its saved arrangement', async t => {
  ...
  const rect = await node(page, 'ryn').boundingBox().then(required); await page.mouse.move(rect.x + 20, rect.y + 20); await page.mouse.down(); await page.mouse.move(rect.x + 100, rect.y + 80, { steps: 3 });
  ...
  assert.deepEqual(await page.locator('codex-campaign-graph').evaluate(graph => (graph as GraphElement).positions.get('ryn')!), { x: 10, y: 20 });
});
...
async function typedPosition(page: Page, kind: string, key: string) {
  return page.locator('codex-campaign-graph').evaluate((graph, id) => (graph as GraphElement).positions.get(id)!, typedKey(kind, key));
}
...
for (const mode of ['factions', 'mysteries']) for (const mobile of [false, true]) test(`${mode} restores original cards, links and navigation (${mobile ? 'phone' : 'desktop'})`, async t => {
  ...
  if (mode === 'factions') {
    ...
    assert.equal(await hub.locator('.cm-fact').textContent().then(required), '2 characters');
    const style = await hub.evaluate(element => ({ width: (element as HTMLElement).offsetWidth, radius: getComputedStyle(element.querySelector('.cm-cloud')!).borderRadius,
    ...
    assert.match(await typedNode(page, 'character', 'mira').textContent().then(required), /Under command: Ryn/);
    ...
    assert.match(await typedNode(page, 'character', 'ryn').textContent().then(required), /2 mysteries/);
    assert.equal(await typedNode(page, 'mystery', 'gate').locator('.cm-hint').textContent().then(required), 'Who opened the northern gate?');
    assert.doesNotMatch(await page.locator('.cm-shell').textContent().then(required), /secret|An answer is not a preview|\[object Object\]/);
  ...
  await page.waitForFunction(() => {
    const graph = document.querySelector('codex-campaign-graph')!, viewport = document.querySelector<HTMLElement>('.cm-viewport')!.getBoundingClientRect();
...
test('same IDs in different collections keep separate cards, links and saved positions', async t => {
  ...
  await publish(page, () => {
    ...
    fixtureRecord<{ characters: string[] }>(collection('mysteries'), 'gate').value.characters.push('gate');
  });
  await switchMode(page, 'mysteries');
...
for (const reducedMotion of ['reduce', 'no-preference'] as const) test(`touch input arranges a faction card without navigating or mutating content (${reducedMotion})`, async t => {
  ...
  const before = await typedPosition(page, 'character', 'mira'), rect = await card.boundingBox().then(required);
  const zoom = await page.locator('codex-campaign-graph').evaluate(graph => (graph as GraphElement).zoom);
...
test('anonymous Czech readers can use every preserved Mind Palace URL', async t => {
  ...
  for (const [hash, mode] of [['palac', 'factions'], ['frakce', 'factions'], ['tajemstvi', 'mysteries']]) {
    ...
    assert.equal(await page.locator('.map-mode-btn[aria-current="page"]').textContent().then(required), mode === 'factions' ? 'Frakce' : 'Záhady');
  }
});

async function holdDrag(page: Page, key: string, dx: number, dy: number) {
  const rect = await node(page, key).boundingBox().then(required);
  const x = rect.x + 20, y = rect.y + 20;
  await page.mouse.move(x, y); await page.mouse.down(); await page.mouse.move(x + dx, y + dy);
}
async function waitForRest(page: Page) { await page.locator('.cm-viewport[data-motion="idle"]').waitFor(); }
...
for (const reducedMotion of ['reduce', 'no-preference'] as const) test(`collision movement keeps the dropped point exact and saves displaced cards (${reducedMotion})`, async t => {
  const { page } = await fixture(t, { reducedMotion });
  const before = await node(page, 'mira').boundingBox().then(required);
  await holdDrag(page, 'ryn', 330, 0);
  await page.waitForFunction(before => document.querySelector<HTMLElement>('.cm-node[data-key="mira"]')!.getBoundingClientRect().x > before.x + 5, before);
  assert.deepEqual(await positions(page), savedPositions);
  await page.mouse.up(); await waitForRest(page);
...
test('Escape restores the complete pre-drag arrangement after other cards have been displaced', async t => {
  ...
  assert.deepEqual(await positions(page), savedPositions);
  const restored = await node(page, 'mira').boundingBox().then(required); assert.equal(restored.x, before.x); assert.equal(restored.y, before.y);
...
test('another tab wins over post-release settling without a delayed overwrite', async t => {
  ...
  assert.deepEqual(await page.locator('codex-campaign-graph').evaluate(graph => Object.fromEntries((graph as GraphElement).positions)), replacement);
});
...
test('a failed settled save keeps the final arrangement available for retry', async t => {
  ...
  const settled = await page.locator('codex-campaign-graph').evaluate(graph => Object.fromEntries((graph as GraphElement).positions));
