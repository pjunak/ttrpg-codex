...
export async function exercisePlannerSelection({ t, open, admin, csrf, output }) {
  ...
  const records = async dataId => (await jsonResponse(await admin.post(`${base}/query`, { headers, data: { contractVersion: 'addon-data-query.v1', kind: 'collection', dataId, limit: 200, where: [] } }))).documents;
  const put = (dataId, value) => ({ operation: 'put', kind: 'collection', dataId, key: value.id, expectedRevision: 0, value });
  const transact = async mutations => jsonResponse(await admin.post(`${base}/transactions`, { headers, data: { contractVersion: 'addon-data-transaction.v1', mutations } }));
  const item = (id, kind = 'event', parentId = 'selection-scope') => ({ id, title: id, kind, parentId, schemaVersion: 3, summary: '', objective: '', body: '', setup: '', resolution: '', tags: [], updatedAt: 1, ...(kind === 'event' ? { eventType: 'story' } : {}) });
  await transact([
  ...
  const card = id => page.locator(`[data-item-id="selection-${id}"]`), viewport = page.locator('.dm-planner-viewport');
  const actions = page.getByRole('group', { name: 'Selection actions', exact: true });
  let writes = []; page.on('request', request => { if (request.url().endsWith('/data/transactions')) writes.push(request.postDataJSON()); });
  ...
  let positions = (await records('planning_views')).find(record => record.key === 'scope-selection-scope').value.positions;
  ...
  positions = (await records('planning_views')).find(record => record.key === 'scope-selection-scope').value.positions;
  ...
  const stage = await page.locator('.dm-planner-stage').boundingBox();
  ...
  assert.equal(await title.evaluate(input => input === document.activeElement), true, await dialog.evaluate(element => document.activeElement?.outerHTML));
  ...
  const point = await flow.evaluate(path => { const p = path.getPointAtLength(path.getTotalLength() / 2).matrixTransform(path.getScreenCTM()); return { x: p.x, y: p.y }; });
  ...
  assert.ok((await records('planning_items')).some(record => record.key === 'selection-child'));
  ...
  assert.ok(!(await records('planning_items')).some(record => ['selection-a', 'selection-child', 'selection-unseen'].includes(record.key)));
