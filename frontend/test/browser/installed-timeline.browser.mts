...
import type { APIRequestContext, Browser, Page, Locator, FrameLocator } from 'playwright';
...
import { installTimelinePackage } from './installed-timeline-fixture.mts';
...
function widget(page: Page, id: string, mode: string, slot: string, eventKey?: string) {
...
async function details(widget: Locator | FrameLocator) { return JSON.parse(await widget.locator('output').textContent().then(required)); }
...
for (const mode of ['integrated', 'isolated']) test(`installed ${mode} timeline slots retain widget state, role filtering and the original board`, async t => {
  ...
  assert.deepEqual(initial.events.map((event: { key: string }) => event.key), ['arrival']); assert.equal(JSON.stringify(initial).includes('northern gate'), false);
  ...
  assert.ok((await details(header)).events.some((event: { key: string }) => event.key === 'secret-event'));
  ...
  assert.deepEqual((await details(playerHeader)).events.map((event: { key: string }) => event.key), ['arrival', 'dinner']);
...
test('a retained timeline draft does not share newly hidden event references with add-ons', async t => {
  ...
  const campaign = await jsonResponse(await admin.get('/api/campaign'));
  const dinner = campaign.collections.find((collection: FixtureCollection) => collection.name === 'events').records.find((record: FixtureRecord) => record.key === 'dinner');
  await jsonResponse(await admin.post('/api/campaign/transactions', { headers: { 'X-Codex-CSRF': csrf }, data: { contractVersion: 'campaign-mutation.v1', mutations: [
  ...
  assert.deepEqual((await details(header)).events.map((event: { key: string }) => event.key), ['arrival']);
  page.on('dialog', dialog => dialog.accept()); await page.getByRole('button', { name: 'Cancel', exact: true }).click();
