...
import type { FixtureCollection, FixtureRecord, InstalledFixture } from './fixture-types.mts';
import assert from 'node:assert/strict';
import { resolve } from 'node:path';
import { jsonResponse } from './installed-graph-fixture.mts';

export async function exerciseCzechInterface({ t, open, admin, csrf, output, mobile }: Pick<InstalledFixture, 't' | 'open' | 'admin' | 'csrf' | 'output' | 'mobile'>) {
  ...
  assert.equal(await page.locator('.addon-navigation-link[href="#/addons/dm-tools/planner"]').textContent().then(required), 'Plánovač příběhu');
  ...
  const create = async (button: string, title: string) => {
    ...
    return (await records('planning_items')).find((record: FixtureRecord) => record.value.title === title);
  };
  ...
  const card = (record: FixtureRecord) => page.locator(`.dm-plan-card[data-item-id="${record.key}"]`);
  ...
  const flow = (await records('planning_flow_links')).find((record: FixtureRecord) => record.value.sourceId === quest.key && record.value.targetId === event.key);
  ...
  const savedReference = (await records('planning_references')).find((record: FixtureRecord) => record.value.itemId === quest.key);
  ...
  assert.ok((await records('planning_flow_links')).some((record: FixtureRecord) => record.key === flow.key));
  await page.reload(); await card(event).waitFor();
  assert.equal(await page.locator('.dm-planner-shell h1').textContent().then(required), 'Plánovač příběhu');
  assert.equal((await records('dm_notes')).find((record: FixtureRecord<{ anchorIds: string[] }>) => record.value.anchorIds.includes(quest.key)).value.body, 'Private authored text');
