...
async function put(key: string, value: unknown, revision: number) { return jsonResponse(await admin.post(`${base}/transactions`, { headers: { 'X-Codex-CSRF': csrf }, data: { contractVersion: 'addon-data-transaction.v1', mutations: [{ operation: 'put', kind: 'record-extension', dataId: 'dnd-sheets', key, expectedRevision: revision, value }] } })); }
async function get(key: string) { return jsonResponse(await admin.post(`${base}/get`, { data: { contractVersion: 'addon-data-get.v1', kind: 'record-extension', dataId: 'dnd-sheets', key } })); }
...
test('campaign recovery refreshes installed Sheets and preserves unsaved add-on drafts', { skip: !archivePath }, async t => {
  ...
  assert.equal(await sheet.locator('.dse-hp .dse-number').first().textContent().then(required), '5');
  const restore = async () => {
  ...
  await restore();
  await page.waitForFunction(() => document.querySelector<HTMLElement>('.dse-hp .dse-number')?.textContent === '21');
...
async function open(t: TestContext, key: string, mobile = false, role = 'dm', configure: (page: Page) => Promise<unknown> = async () => {}) {
...
async function saved(page: Page) { await page.locator('.dnd-save-status').getByText('Saved.', { exact: true }).waitFor(); }
...
for (const mobile of [false, true]) test(`character profile and installed sheet preserve editing state on ${mobile ? 'phone' : 'desktop'}`, { skip: !archivePath }, async t => {
  ...
  const save = await page.getByRole('button', { name: 'Save entry', exact: true }).boundingBox().then(required);
...
async function write(page: Page, action: () => Promise<unknown>) {
...
for (const mobile of [false, true]) test(`installed Sheets restores Compact/Classic play on ${mobile ? 'phone' : 'desktop'}`, { skip: !archivePath }, async t => {
  ...
  assert.match(await sheet.locator('.dse-attack').textContent().then(required), /Walking staff.*\+3.*1d6/);
  ...
  assert.equal(await sheet.locator('.dse-hp .dse-number').first().textContent().then(required), '20');
  assert.equal((await get(key)).value.inventory.find((item: Record<string, unknown>) => item.id === 'rope').location, 'ready');
  assert.deepEqual((await get(key)).value.homebrew, { clue: 'blue lantern' });
...
test('installed Sheets retains a failed draft, retries and handles concurrent edits without overwrite', { skip: !archivePath }, async t => {
  ...
  await page.waitForFunction(() => !document.querySelector<HTMLElement>('.dnd-save-status [role=alert]'));
  ...
  await page.waitForFunction(() => document.querySelector<HTMLInputElement>('.addon-dnd-sheets [aria-label="Current HP"]')?.value === '8');
...
for (const mobile of [false, true]) test(`installed Sheets provider diagnostics preserve standalone values in ${mobile ? 'Czech on phone' : 'English on desktop'}`, { skip: !archivePath }, async t => {
  ...
  const page = await open(t, key, mobile, mobile ? 'player' : 'dm', async (page: Page) => {
  ...
  await page.waitForFunction(() => !document.querySelector<HTMLButtonElement>('.dnd-provider-status button')?.disabled);
...
test('installed Sheets provider reconnection retains a failed Czech draft', { skip: !archivePath }, async t => {
  ...
  const page = await open(t, key, true, 'dm', async (page: Page) => {
...
test('installed Sheets provider diagnostics recover missing data and compare saved provenance without writes', { skip: !archivePath || !process.env.CODEX_ENGINE_ZIP || !process.env.CODEX_COMPENDIUM_ZIP }, async t => {
  await installReviewedPackage(admin, csrf, 'dnd-engine', await readFile(resolve(required(process.env.CODEX_ENGINE_ZIP))), []);
  ...
  assert.deepEqual(computed.value.inventory, original.value.inventory.map((item: Record<string, unknown>) => ({ notes: '', ...item })));
...
test('installed Sheets uses real catalog labels and keeps scores stable across recalculation', { skip: !archivePath || !process.env.CODEX_ENGINE_ZIP || !process.env.CODEX_COMPENDIUM_ZIP }, async t => {
  ...
  assert.match(await sheet.locator('.dnd-builder-class-row select').first().textContent().then(required), /Fighter/);
  for (let i = 0; i < 2; i++) {
    ...
    assert.ok(document.value.rulesProvider.materialized.resources.some((resource: Record<string, unknown>) => resource.key === 'slot-1'));
  }
  await sheet.getByRole('tab', { name: 'Combat', exact: true }).click();
...
for (const mobile of [false, true]) test(`installed Sheets equipment, spells and reviewed rests persist on ${mobile ? 'phone' : 'desktop'}`, { skip: !archivePath || !process.env.CODEX_ENGINE_ZIP || !process.env.CODEX_COMPENDIUM_ZIP }, async t => {
  ...
  assert.match(await equipment.getByRole('navigation', { name: 'Equipment folders' }).textContent().then(required), /All equipment.*Weapons.*martial.*melee/);
  ...
  assert.equal(state.spells.find((item: Record<string, unknown>) => item.id === 'snapshot:mage-armor').name, 'Mage Armor');
  assert.equal(state.spells.find((item: Record<string, unknown>) => item.id === 'snapshot:mage-armor').level, 1);
...
for (const mobile of [false, true]) test(`installed Sheets spell grants, copying and rituals retain data on ${mobile ? 'phone' : 'desktop'}`, { skip: !archivePath || !process.env.CODEX_ENGINE_ZIP || !process.env.CODEX_COMPENDIUM_ZIP }, async t => {
  ...
  assert.match(await review.textContent().then(required), /100.*50.*Scroll of Alarm/);
  ...
  await sheet.getByRole('button', { name: 'Retry save', exact: true }).waitFor();
  let state = (await get(key)).value;
  assert.equal(state.currency.gp, 100); assert.equal(state.inventory.find((item: Record<string, unknown>) => item.id === 'scroll').qty, 2); assert.equal(await unloadBlocked(page), true);
  ...
  assert.equal(state.currency.gp, 50); assert.equal(state.inventory.find((item: Record<string, unknown>) => item.id === 'scroll').qty, 1);
  assert.equal(state.inventory.find((item: Record<string, unknown>) => item.id === 'scroll').notes, 'From the lighthouse'); assert.deepEqual(state.spellbook.wizard, ['alarm']);
  await sheet.getByRole('button', { name: 'Manage class spells', exact: true }).click();
  await sheet.getByRole('searchbox', { name: 'Search spells', exact: true }).fill('Alarm');
...
for (const mobile of [false, true]) test(`installed Sheets worn slots replace armor, retain shields and attune on ${mobile ? 'phone' : 'desktop'}`, { skip: !archivePath || !process.env.CODEX_ENGINE_ZIP || !process.env.CODEX_COMPENDIUM_ZIP }, async t => {
  ...
  const page = await open(t, key, mobile), sheet = page.locator('.dnd-sheet-shell');
  await sheet.getByRole('button', { name: 'Edit sheet', exact: true }).click();
  async function fill(slot: string, name: string) {
  ...
  let state = (await get(key)).value;
  assert.equal(state.ac, 18); assert.equal(state.inventory.find((item: Record<string, unknown>) => item.id === 'leather-armor').location, 'pack');
  assert.equal(state.inventory.find((item: Record<string, unknown>) => item.id === 'shield').location, 'equipped');
  await fill('Attunement', 'Cloak of Protection');
  state = (await get(key)).value; assert.equal(state.inventory.find((item: Record<string, unknown>) => item.id === 'cloak-of-protection').attuned, true);
  ...
  state = (await get(key)).value;
  assert.equal(state.ac, 16); assert.equal(state.inventory.find((item: Record<string, unknown>) => item.id === 'shield').location, 'pack'); assert.equal(state.inventory.find((item: Record<string, unknown>) => item.id === 'cloak-of-protection').attuned, false);
  assert.equal(state.inventory.find((item: Record<string, unknown>) => item.id === 'chain-mail').notes, 'Keep engraving'); assert.equal(state.homebrew.clue, 'blue lantern');
...
test('installed Sheets reviews known spell swaps and persists their class-level history', { skip: !archivePath || !process.env.CODEX_ENGINE_ZIP || !process.env.CODEX_COMPENDIUM_ZIP }, async t => {
  ...
  assert.match(await sheet.locator('.dnd-spell-history').textContent().then(required), /Warlock 5: Hex → Armor of Agathys/);
...
for (const mobile of [false, true]) test(`installed Builder restores progress navigation, advanced choices and rewards on ${mobile ? 'phone' : 'desktop'}`, { skip: !archivePath || !process.env.CODEX_ENGINE_ZIP || !process.env.CODEX_COMPENDIUM_ZIP }, async t => {
  ...
  assert.equal(await level3.evaluate(node => (node as HTMLDialogElement).open), true);
  ...
  assert.match(await maneuvers.textContent().then(required), /3 \/ 3 selected/);
  ...
  async function addReward(feat: string, name: string, note: string) {
...
for (const mobile of [false, true]) test(`installed Czech sheet completes Builder, equipment, spell and rest workflows on ${mobile ? 'phone' : 'desktop'}`, { skip: !archivePath || !process.env.CODEX_ENGINE_ZIP || !process.env.CODEX_COMPENDIUM_ZIP }, async t => {
  ...
  await put(key, { ...initial.value, classes: [{ classId: 'wizard', level: 5 }], feats: [{ featId: 'magic-initiate' }], species: 'Elf', lineage: 'high-elf', background: 'Acolyte',
    currency: { ...initial.value.currency, gp: 100 }, inventory: [...initial.value.inventory, { id: 'scroll', name: 'Scroll of Alarm', qty: 2, location: 'pack', notes: 'From the lighthouse' }] }, initial.revision);
  const page = await open(t, key, mobile, 'dm', (page: Page) => page.addInitScript(() => localStorage.setItem('codex_lang', 'cs')));
...
test('installed Czech sheet reviews spell swaps and preserves saved references', { skip: !archivePath || !process.env.CODEX_ENGINE_ZIP || !process.env.CODEX_COMPENDIUM_ZIP }, async t => {
  ...
  await put(key, { ...initial.value, classes: [{ classId: 'warlock', level: 5 }], className: 'Warlock', preparedSpells: { warlock: ['hex'] } }, initial.revision);
  const page = await open(t, key, true, 'dm', (page: Page) => page.addInitScript(() => localStorage.setItem('codex_lang', 'cs'))), sheet = page.locator('.dnd-sheet-shell');
  await sheet.getByRole('button', { name: 'Upravit deník', exact: true }).click();
