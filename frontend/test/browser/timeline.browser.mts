import { required, fixtureCollection } from './fixture-types.mts';
...
const streams = new Set<import("node:http").ServerResponse>(), output = fileURLToPath(new URL('../../test-results/timeline/', import.meta.url));
const collection = (name: string) => fixtureCollection(campaign, name);
const findEvent = (key: string) => collection('events').records.find(record => record.key === key) as FixtureRecord | undefined;
const event = (key: string) => required(findEvent(key));
const card = (page: Page, key: string) => page.locator(`.tl-card[data-key="${key}"]`);
const column = (page: Page, sitting: number) => page.locator(`.tl-col[data-sitting="${sitting}"]`);
const ids = (page: Page, sitting: number) => column(page, sitting).locator('.tl-card').evaluateAll(cards => cards.map(card => (card as HTMLElement).dataset.key));
before(async () => {
...
async function fixture(t: TestContext, { role = 'player', mobile = false, locale = 'en-US' } = {}) {
  ...
  await page.route('**/api/campaign/transactions', async route => {
    ...
    for (const mutation of body.mutations) assert.equal(mutation.expectedRevision, findEvent(mutation.key)?.revision ?? 0);
    const results = [];
    for (const mutation of body.mutations) {
      const record = findEvent(mutation.key);
...
for (const mobile of [false, true]) {
  test(`timeline keeps original session columns, stacked cards and scrolling (${mobile ? 'phone' : 'desktop'})`, async t => {
    ...
    assert.equal(await column(page, 2).locator('.tl-col-empty').textContent().then(required), 'No events in this session yet.');
    assert.match(await column(page, 1).getAttribute('class').then(required), /tl-col-stacked/);
    assert.match(await card(page, 'event-0').textContent().then(required), /Ryn, Mira/);
    assert.match(await card(page, 'event-0').textContent().then(required), /Northern Gate/);
    const shell = await page.locator('.tl-shell').boundingBox().then(required), col = await column(page, 1).boundingBox().then(required);
    ...
    if (mobile) {
      const slider = page.getByRole('slider', { name: 'Scroll through sessions' }); await slider.fill(await slider.getAttribute('max').then(required));
...
test('native drag creates a draft and rejects a revision changed after dragstart', async t => {
  ...
  assert.equal(writes.length, 1); assert.match(await card(page, 'event-0').textContent().then(required), /Event 0/);
...
test('timeline creates, edits and deletes events with the common revision-protected form', async t => {
  ...
  assert.equal(findEvent('event-0'), undefined); assert.equal(writes.at(-1)![0].expectedRevision, 4);
});
