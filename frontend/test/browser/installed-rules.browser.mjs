...
let directory, host, admin, csrf, base, engine, sheets, compendium, hostOutput = '';
before(async () => {
  if (!enabled) return;
  [engine, sheets, compendium] = await Promise.all(paths.map(path => readFile(resolve(path))));
...
const hydrate = decisions => call('hydrate', { contractVersion: 'rules-engine-hydrate.v1', decisions });
const plan = decisions => call('builder-plan', { contractVersion: 'rules-engine-builder-plan.v1', decisions });
...
test('installed Compendium drives complete class hydration and Builder through Sheets', { skip: !enabled }, async () => {
  ...
  for (const record of classes.records) {
    ...
    assert.ok(result.sheet.resources.some(resource => resource.kind === 'hitdice'), record.id);
  ...
  const options = await plan(decisions); assert.ok(options.plan.creationAbilityChoices.some(choice => choice.id === 'bgasi'));
  ...
  assert.equal(applied.available, true); assert.equal(applied.decisions.abilityGrants.find(grant => grant.id === 'bgasi').assign.INT, 2);
...
function changedCompendium(archive) {
  ...
  const manifest = JSON.parse(files['addon.json']); manifest.version = '3.0.1'; manifest.content[0].revision = 'installed-rules-fixture-2'; files['addon.json'] = JSON.stringify(manifest);
  const recordPath = 'data/phb/classes/wizard.json', record = JSON.parse(files[recordPath]); assert.equal(record.hitDie, 'd6');
