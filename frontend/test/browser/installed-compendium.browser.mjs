...
import { jsonResponse, installReviewedPackage, zip } from './installed-graph-fixture.mjs';
...
let directory, host, browser, admin, csrf, origin, archive, hostOutput = '';
before(async () => {
...
async function open(t, role = 'dm', mobile = false, locale = 'en', navigate = true) {
...
async function fits(page) { assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), true); }
async function go(page, query) { await page.goto(`/${route}${query}`); await page.locator('.comp-reading-pane h1').waitFor(); }
...
for (const mobile of [false, true]) for (const role of ['dm', 'player']) test(`installed compendium restores browsing and reading for ${role} on ${mobile ? 'phone' : 'desktop'}`, { skip: !archivePath }, async t => {
  ...
  await page.waitForFunction(() => Number(document.querySelector('.dnd-compendium')?.dataset.fixtureRefreshes) > 0);
  ...
  assert.match(await pane.locator('.codex-fact-grid').textContent(), /Saving Throws/); await fits(page);
  await page.screenshot({ path: resolve(output, `monster-${role}-${mobile ? 'phone' : 'desktop'}.png`), fullPage: true });
  await go(page, '?kind=spell&id=shield'); const spellURL = page.url(); assert.match(await pane.locator('.codex-fact-grid').textContent(), /Casting Time/);
  await go(page, '?kind=armor&id=shield'); assert.notEqual(page.url(), spellURL); assert.match(await pane.locator('.codex-fact-grid').textContent(), /Armor Class/);
...
function replacement(archive) {
  const end = archive.lastIndexOf(Buffer.from([0x50,0x4b,0x05,0x06])), files = Object.create(null); assert.ok(end >= 0);
  ...
  const manifest = JSON.parse(files['addon.json']); manifest.version = '3.0.1'; files['addon.json'] = JSON.stringify(manifest);
...
test('installed compendium replaces constructors and can reactivate the same generation', { skip: !archivePath }, async t => {
  ...
  await page.waitForFunction(first => { const node = document.querySelector('.dnd-compendium'); return node && node.localName !== first && node.textContent.includes('Fireball'); }, first);
