...
export async function exercisePlannerConcurrency({ t, open, admin, csrf }) {
  ...
  const item = (id, parentId = null, kind = 'event') => ({ id, title: id, kind, parentId, schemaVersion: 3, summary: '', objective: '', body: '', setup: '', resolution: '', tags: [], updatedAt: 1, ...(kind === 'event' ? { eventType: 'story' } : {}) });
  const flow = (id, sourceId, targetId) => ({ id, schemaVersion: 3, sourceId, targetId, kind: 'continues', label: id, updatedAt: 1 });
  await transact([
  ...
  assert.ok((await records('planning_items')).some(record => record.key === 'race-parent'));
  assert.ok((await records('planning_items')).some(record => record.key === 'race-unseen-child'));
  ...
  assert.ok((await records('planning_flow_links')).some(record => record.key === 'race-bc'));
  assert.ok((await records('planning_consequences')).some(record => record.key === 'race-unseen-consequence'));
  ...
  assert.equal((await records('planning_flow_links')).some(record => record.value.sourceId === 'race-c'), false);
