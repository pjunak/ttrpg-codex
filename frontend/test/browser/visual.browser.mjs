...
import { visualFixturePlugin } from './visual-fixture.mjs';

let server, referenceServer, browser, origin, referenceOrigin;
...
before(async () => {
  ...
  referenceOrigin = `http://127.0.0.1:${referenceServer.httpServer.address().port}`;
  browser = await chromium.launch({ headless: true });
});
after(async () => { await browser?.close(); await server?.close(); await referenceServer?.close(); });

async function fixture(t, viewport, role, addon = false) {
...
async function style(page, selector, properties) {
...
async function compare(page, reference, actual, original, properties) {
...
for (const [name, viewport] of Object.entries({ desktop: { width: 1440, height: 1000 }, mobile: { width: 390, height: 844 } })) {
  test(`classic ${name} matches preserved v1 typography, surfaces and card geometry`, async t => {
    ...
    const card = await page.locator('.record-row-mark').first().boundingBox();
    ...
    const rail = await page.locator('.record-side').boundingBox();
    const reading = await page.locator('.record-reading').boundingBox();
...
test('mobile drawer stays accessible and preserves dirty record edits', async t => {
  ...
  assert.equal(await page.locator('.campaign-sidebar').evaluate(element => element.inert), true);
...
test('desktop sidebar, phone menu and settings remain usable across resize', async t => {
  ...
  assert.equal(await page.locator('.campaign-sidebar').evaluate(element => element.inert), false);
  assert.equal(await page.locator('[data-menu-toggle]').isVisible(), false);
  await page.setViewportSize({ width: 320, height: 780 });
  await page.locator('[data-menu-toggle]').click();
  await page.waitForFunction(() => document.querySelector('.campaign-sidebar').getBoundingClientRect().x === 0);
...
test('one add-on navigation outlet survives desktop and mobile transitions', async t => {
  ...
  await link.waitFor().catch(async error => {
    t.diagnostic(await page.locator('.account-panel small').getAttribute('title'));
    throw error;
  });
  assert.equal(await link.count(), 1);
  await link.click();
  await page.locator('visual-fixture-addon').waitFor();
  assert.equal(await page.locator('visual-fixture-addon').textContent(), 'Synthetic add-on page');
...
test('skip link moves focus into the campaign without changing its route', async t => {
  ...
  assert.equal(await page.locator('#campaign-title').textContent(), 'Asurai');
});
