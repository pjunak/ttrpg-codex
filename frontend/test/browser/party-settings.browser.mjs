...
const party = () => collection('settings').records.find(record => record.key === 'playerParty');
const panel = page => page.locator('codex-party-settings');

async function fixture(t, { mobile = false, role = 'dm', reject = false, locale = 'en-US', absent = false, malformed = false } = {}) {
  ...
  await page.route('**/api/campaign/transactions', async route => {
    ...
    if (reject || mutation.expectedRevision !== (party()?.revision ?? 0)) return route.fulfill({ status: 409, json: { error: 'conflict' } });
...
async function publish(page, name, change) {
...
for (const mobile of [false, true]) test(`party settings save and reach their readers (${mobile ? 'phone' : 'desktop'})`, async t => {
  ...
  await publish(page, 'characters', () => { collection('characters').records[0].value.name = 'Ryn II'; });
  ...
  assert.equal(await badge.textContent(), '🦉 Night Owls');
  ...
  assert.equal(await page.locator('[name="faction"] option[value="party"]').textContent(), '🦉 Night Owls');
  await page.goto(`${origin}/#/party`);
  await page.locator('#party-heading').waitFor();
  assert.match(await page.locator('#party-heading').textContent(), /🦉 Night Owls/);
  assert.equal(await page.locator('.party-member .portrait-fallback').first().textContent(), '🦉');
  assert.match(await page.locator('.party-member .party-portrait').first().getAttribute('style'), /153,\s*204,\s*255/);
});
...
test('clean forms follow live refresh, while a deleted party setting cannot consume an old draft', async t => {
  ...
  const name = panel(page).getByLabel('Name', { exact: true });
  await publish(page, 'settings', () => { party().revision++; party().value.name = 'Remote name'; });
  await page.waitForFunction(() => document.querySelector('[name="partyName"]')?.value === 'Remote name');
...
for (const role of ['', 'player']) test(`${role || 'anonymous'} readers see shared party identity without settings write controls`, async t => {
  ...
  await page.goto(`${origin}/#/party`);
  await page.locator('#party-heading').waitFor();
  assert.match(await page.locator('#party-heading').textContent(), /⚔ Ember Company/);
  assert.equal(writes.length, 0);
});
