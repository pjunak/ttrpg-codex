...
import { closePlannerEditor } from './installed-planner-dialog-fixture.mts';
import { unloadBlocked } from './installed-planner-navigation-fixture.mts';

async function campaign(admin: APIRequestContext, csrf: string) {
  ...
  return {
    records: async (dataId: string) => (await jsonResponse(await admin.post(`${base}/query`, { headers, data: { contractVersion: 'addon-data-query.v1', kind: 'collection', dataId, limit: 200, where: [] } }))).documents,
    transact: async (mutations: unknown[]) => jsonResponse(await admin.post(`${base}/transactions`, { headers, data: { contractVersion: 'addon-data-transaction.v1', mutations } })),
  };
}
...
const item = (id: string, kind: string, parentId: string | null = null) => ({ id, title: id, kind, parentId, schemaVersion: 3, summary: '', objective: '', body: '', setup: '', resolution: '', tags: [], updatedAt: 1, ...(kind === 'event' ? { eventType: 'story' } : {}), ...(kind === 'branch' ? { branchType: 'decision' } : {}) });

export async function exercisePlannerActions({ t, open, admin, csrf, output, mobile }: Pick<InstalledFixture, 't' | 'open' | 'admin' | 'csrf' | 'output' | 'mobile'>) {
  ...
  const card = (id: string) => page.locator(`[data-item-id="${id}"]`), port = (id: string) => page.locator(`[data-flow-port="${id}"]`);
  const writes: FixtureTransaction[] = []; page.on('request', request => { if (request.url().endsWith('/data/transactions')) writes.push(request.postDataJSON()); });
  ...
  const created = (await records('planning_items')).filter((record: FixtureRecord) => record.value.title === `Created ${suffix}`); assert.equal(created.length, 1); assert.equal(created[0].value.parentId, scope);
  ...
  await page.keyboard.press('Shift+Tab'); assert.equal(await help.evaluate(element => element.contains(document.activeElement)), true, await help.evaluate(element => JSON.stringify({ active: document.activeElement?.outerHTML.slice(0, 1000), open: (element as HTMLDialogElement).open, dialogs: [...document.querySelectorAll('dialog')].map(value => value.outerHTML.slice(0, 350)) })));
  ...
  assert.ok((await records('planning_flow_links')).some((record: FixtureRecord) => record.value.sourceId === a && record.value.targetId === b && record.value.kind === 'continues'));
  ...
  assert.ok((await records('planning_flow_links')).some((record: FixtureRecord) => record.value.sourceId === c && record.value.targetId === b && record.value.kind === 'option'));
  ...
  const reset = (await records('planning_views')).find((record: FixtureRecord) => record.key === `scope-${scope}`); assert.deepEqual(reset.value.positions, {});
  assert.deepEqual((await records('planning_views')).find((record: FixtureRecord) => record.key === `scope-${a}`), layouts.find((record: FixtureRecord) => record.key === `scope-${a}`));
  ...
  if (!mobile) {
    ...
    assert.equal(await page.getByRole('button', { name: 'Reset zoom to 100%', exact: true }).textContent().then(required), '200%');
    await viewport.evaluate(element => { element.scrollLeft = 0; element.scrollTop = 0; });
    await port(c).scrollIntoViewIfNeeded(); const start = await port(c).boundingBox().then(required);
    ...
    const from = await port(a).boundingBox().then(required), to = await card(c).boundingBox().then(required);
    ...
    assert.ok((await records('planning_flow_links')).some((record: FixtureRecord) => record.value.sourceId === a && record.value.targetId === c));
  }
}

export async function exercisePlannerCreationFailures({ t, open, admin, csrf }: Pick<InstalledFixture, 't' | 'open' | 'admin' | 'csrf'>) {
  ...
  assert.equal((await records('planning_items')).filter((record: FixtureRecord) => record.value.title === 'Lost creation response').length, 1);
  ...
  const { promise: held, resolve: release } = Promise.withResolvers<void>(), { promise: pending, resolve: received } = Promise.withResolvers<void>(); t.after(() => release());
  ...
  assert.equal((await records('planning_items')).filter((record: FixtureRecord) => record.value.title === 'Creation with failed confirming read').length, 1); await closePlannerEditor(page);
