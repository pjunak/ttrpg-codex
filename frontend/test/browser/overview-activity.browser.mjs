...
async function open(t, role, mobile = false) {
...
async function put(client, token, key, value, revision = 0) {
...
async function record(key) {
  ...
  return data.collections.find(collection => collection.name === 'locations').records.find(record => record.key === key);
}
...
test('empty recent history is explained to signed-in users without requiring an add-on', async t => {
  ...
  assert.equal(await player.page.locator('.recent-section .empty-state').textContent(), 'No changes yet. Newly created and edited entries will appear here.');
...
for (const mobile of [false, true]) test(`overview activity follows player creations and edits with role filtering on ${mobile ? 'phone' : 'desktop'}`, async t => {
  ...
  const session = await player.page.locator('.session-section').boundingBox(), recent = await player.page.locator('.recent-section').boundingBox();
