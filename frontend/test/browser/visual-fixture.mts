...
import type { FixtureCampaign, FixtureRecord } from './fixture-types.mts';
export const visualCampaign: FixtureCampaign = {
  ...
  collections: ['characters','relationships','locations','events','mysteries','factions','deletedDefaults','pantheon','artifacts','settings','historicalEvents','campaign','pets'].map(name => ({
    ...
    } as Record<string, FixtureRecord<unknown>[]>)[name] ?? [],
  })),
};
...
export function visualFixturePlugin({ getCampaign = () => visualCampaign, onStream = () => {} }: {
  getCampaign?: () => FixtureCampaign;
  onStream?: (response: import('node:http').ServerResponse) => void;
} = {}): import('vite').Plugin {
  const streams = new Set<import("node:http").ServerResponse>();
  const configure = (server: Pick<PreviewServer, 'middlewares'>) => {
      server.middlewares.use((request, response, next) => {
        ...
        } as Record<string, unknown>)[request.url];
