...
let directory, host, browser, admin, origin, imageURL, hostOutput = '';
before(async () => {
...
for (const mobile of [false, true]) test(`Go host serves visible markers and EXIF map tiles on ${mobile ? 'phone' : 'desktop'}`, async t => {
  ...
  const reads = [], errors = []; page.on('request', request => reads.push(new URL(request.url()).pathname));
  ...
  await page.waitForFunction(() => {
    ...
    return markers.length > 0 && tiles.length > 0 && [...markers, ...tiles].every(image => image.complete && image.naturalWidth > 0)
