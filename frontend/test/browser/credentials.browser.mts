...
let directory: string, binary: string, host: ChildProcessByStdio<null, Readable, Readable>, browser: Browser, admin: APIRequestContext, origin: string, address: string, csrf: string, hostOutput = '';
...
before(async () => {
  ...
  const probe = createServer(); probe.listen(0, '127.0.0.1'); await once(probe, 'listening'); address = `127.0.0.1:${(probe.address() as AddressInfo).port}`; await new Promise(resolve => probe.close(resolve)); origin = `http://${address}`;
...
async function open(t: TestContext, mobile = false, password: string | null = dmPassword) {
...
const form = (page: Page, role: string) => page.locator(`.settings-password-card[data-role="${role}"]`);
async function fill(page: Page, role: string, current: string, next: string, confirmation = next) {
...
async function submit(page: Page, role: string) {
...
test('password writes block navigation and uncertain outcomes retain the reviewing DM session', async t => {
  ...
  const { promise: blocked, resolve: release } = Promise.withResolvers<void>();  const { promise: waiting, resolve: entered } = Promise.withResolvers<void>();
...
test('password settings stay private and saved credentials survive restart and offline reset', async t => {
  ...
  assert.equal(campaign.collections.find((value: FixtureCollection) => value.name === 'characters')!.records.find((value: FixtureRecord) => value.key === 'keeper').value.notes, 'Retain this campaign.');
});
