...
async function record(key) {
  ...
  return data.collections.find(c => c.name === 'characters').records.find(r => r.key === key);
}
async function put(key, value, revision = 0) {
...
async function open(t, key, mobile = false, role = 'player') {
...
async function picture(page, color = 'green') {
...
async function edit(page) { await page.getByRole('button', { name: 'Edit', exact: true }).click(); }
...
for (const mobile of [false, true]) test(`portrait draft uploads, replaces and removes through the real host on ${mobile ? 'phone' : 'desktop'}`, async t => {
  ...
  const uploads = []; page.on('request', r => { if (r.method() === 'POST' && r.url().includes('/api/media/character-portrait/')) uploads.push(r.url()); });
  await edit(page);
  ...
  await page.waitForFunction(() => document.querySelector('.record-portrait')?.naturalWidth === 96);
...
test('failed uploads and rejected record saves keep the portrait draft and original record', async t => {
  ...
  assert.match(await page.getByRole('img', { name: 'Portrait preview' }).getAttribute('src'), /^blob:/);
