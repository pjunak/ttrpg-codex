...
import { editPlannerCard } from './installed-planner-dialog-fixture.mts';
import assert from 'node:assert/strict';
import { resolve } from 'node:path';
import { jsonResponse } from './installed-graph-fixture.mts';

export async function exercisePlannerStructure({ t, open, admin, csrf, output }: Pick<InstalledFixture, 't' | 'open' | 'admin' | 'csrf' | 'output'>) {
  ...
  for (const mobile of [false, true]) {
    const prefix = mobile ? 'structure-phone-' : 'structure-desktop-', id = (name: string) => prefix + name;
    const item = (name: string, kind = 'quest', parent: string | null = null) => ({ id: id(name), schemaVersion: 3, kind, parentId: parent ? id(parent) : null,
