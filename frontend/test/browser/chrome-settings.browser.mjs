...
const streams = new Set(), output = fileURLToPath(new URL('../../test-results/chrome-settings/', import.meta.url));
const collection = name => campaign.collections.find(item => item.name === name);
const setting = key => collection('settings').records.find(item => item.key === key);
...
async function fixture(t, { mobile = false, role = 'dm', locale = 'en', reject = false, failUpload = false, addon = false, malformed = false } = {}) {
  ...
  await page.route('**/api/campaign/transactions', async route => {
    ...
    if (reject || body.mutations.some(item => item.expectedRevision !== (setting(item.key)?.revision ?? 0))) return route.fulfill({ status: 409, json: { error: 'conflict' } });
    const results = [];
...
async function openCategory(page, category) { await page.locator(`[data-category="${category}"]`).click(); }
async function saved(page, key, revision) {
...
async function showMenu(page, mobile) { if (mobile) await page.locator('[data-menu-toggle]').click(); }
...
for (const mobile of [false, true]) test(`branding text, logo and reset reach the shell (${mobile ? 'phone' : 'desktop'})`, async t => {
  ...
  assert.equal(await page.locator('.campaign-brand strong').textContent(), 'Tiamat'); assert.equal(await page.locator('.campaign-sigil').getAttribute('src'), logoUrl);
  ...
  assert.equal(setting('branding').value.logoUrl, ''); assert.match(await page.locator('.campaign-sigil').getAttribute('src'), /logo-default/);
...
test('server conflicts keep uploaded branding and sidebar drafts without changing accepted values', async t => {
  ...
  assert.equal(await page.locator('.campaign-brand strong').textContent(), 'Asurai');
