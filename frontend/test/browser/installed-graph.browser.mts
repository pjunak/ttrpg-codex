...
import { installGraphPackage, jsonResponse } from './installed-graph-fixture.mts';
...
const card = (page: Page, id: string, contribution = 'notes') => page.locator(`.cm-node[data-key=${JSON.stringify(JSON.stringify(['addon', id, contribution, 'node', 'note']))}]`);
...
for (const mode of ['integrated', 'isolated']) test(`installed ${mode} graphs preserve core cards, detail navigation and local movement`, async t => {
  ...
  await note.waitFor(); assert.equal(await note.locator('.cm-name').textContent().then(required), 'Trail note');
  assert.equal(await note.locator('.cm-hint').textContent().then(required), 'Follow the northern road.');
  assert.equal(await page.locator('.cm-node[data-key="captain"] .cm-name').textContent().then(required), 'Captain');
  ...
  const before = await board.boundingBox().then(required); await board.focus(); await page.keyboard.press('ArrowRight');
...
test('late installed models and live refresh preserve explicit zoom and saved positions', async t => {
  ...
  const captain = snapshot.collections.find((collection: FixtureCollection) => collection.name === 'characters').records.find((record: FixtureRecord) => record.key === 'captain');
  await jsonResponse(await admin.post('/api/campaign/transactions', { headers: { 'X-Codex-CSRF': csrf }, data: { contractVersion: 'campaign-mutation.v1', mutations: [
