...
export async function exercisePlannerCanvas({ t, open, admin, csrf, output, mobile }) {
  ...
  const item = (id, kind, parentId) => ({ id, schemaVersion: 3, kind, parentId, title: id, summary: 'Canvas navigation retains the original authored content.', body: '', objective: '', setup: '', resolution: '', tags: [], updatedAt: 1, ...(kind === 'event' ? { eventType: 'story' } : {}) });
  ...
  await controls.getByRole('button', { name: 'Fit', exact: true }).click(); const fitted = await zoom.textContent();
  await page.getByRole('button', { name: 'Campaign', exact: true }).click(); await page.locator('.dm-planner-viewport[data-scope=""]').waitFor(); assert.equal(await zoom.textContent(), '100%');
  await page.getByRole('button', { name: 'Reload planner', exact: true }).click(); await page.locator('.dm-planner-shell[aria-busy="false"]').waitFor();
  ...
  const saved = documents.find(record => record.value.scopeId === quest);
