...
let server, browser, origin, campaign;
...
const identity = () => collection('campaign').records[0];

async function fixture(t, { role = 'dm', mobile = false, reject = false, statuses = true } = {}) {
  ...
  await page.route('**/api/campaign/transactions', async route => {
    ...
    if (reject || body.mutations.some(mutation => mutation.expectedRevision !==
...
test('party creation uses the original defaults, survives live refresh, and saves once', async t => {
  ...
  await page.waitForURL(/#\/characters\/new-companion-/).catch(async error => {
    t.diagnostic(JSON.stringify({ url: page.url(), writes, alert: await page.locator('.application-alert').allTextContents(),
      invalid: await page.locator('.record-editor :invalid').evaluateAll(elements => elements.map(element => ({ name: element.name, error: element.validationMessage }))) }));
    throw error;
  });
  assert.equal(writes.length, 1);
