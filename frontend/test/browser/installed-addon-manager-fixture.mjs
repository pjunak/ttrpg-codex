...
import { graphPackage, jsonResponse } from './installed-graph-fixture.mjs';

export async function exerciseAddonManager({ t, open, admin, csrf, output, mobile }) {
  ...
  await manager.getByRole('button', { name: 'Refresh list', exact: true }).click(); await row.getByRole('button', { name: 'Disable', exact: true }).waitFor();
  ...
  await manager.getByRole('button', { name: 'Refresh list', exact: true }).click(); await row.getByText('Disabled or awaiting activation', { exact: true }).waitFor();
  ...
  await manager.getByRole('button', { name: 'Refresh list', exact: true }).click(); await manager.locator('.addon-manager[aria-busy="false"]').waitFor();
  ...
  await manager.getByRole('button', { name: 'Zkontrolovat ZIP', exact: true }).waitFor();
