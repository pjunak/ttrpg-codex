...
export async function exercisePlannerNavigation({ t, open, admin, csrf }: Pick<InstalledFixture, 't' | 'open' | 'admin' | 'csrf'>) {
  ...
  const item = (id: string) => ({ id, schemaVersion: 3, kind: 'event', eventType: 'story', parentId: null, title: id,
  ...
  for (const mobile of [false, true]) {
    ...
    const pattern = '**/data/transactions';
   const { promise: held, resolve: release } = Promise.withResolvers<void>(), { promise: pending, resolve: received } = Promise.withResolvers<void>();
    t.after(() => release());
    ...
    if (!mobile) {
      const beforeLogout = await jsonResponse(await page.context().request.get('/api/auth'));
      await page.locator('.account-menu button').filter({ hasText: /^Sign out$/u }).evaluate(button => (button as HTMLElement).click());
      assert.deepEqual(await jsonResponse(await page.context().request.get('/api/auth')), beforeLogout);
    }
    assert.equal(await unloadBlocked(page), true);
    release();
