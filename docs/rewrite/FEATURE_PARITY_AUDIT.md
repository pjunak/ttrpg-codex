...
3. **Complete operator control.** Instance sourcebook policy (F07) and provider selection (F08) are implemented across the host/provider/consumer boundary. Package uninstall with retained data (F09) remains open; a data-purge action requires its own decision. Broader F17 reconciliation remains distinct from source eligibility.
