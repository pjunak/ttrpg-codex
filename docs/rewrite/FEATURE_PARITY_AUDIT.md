...
At audit time, release-gate checkboxes were closed while source selection, uninstall, settings/editor outlets and graph capabilities remained deferred. Source selection and uninstall have since been implemented; the other scoped findings remain. The [release-check script](../../scripts/check-release-readiness.mts) checks checkbox/marker state, not whether every legacy action has a working replacement.
...
3. **Complete operator control.** Instance sourcebook policy (F07), provider selection (F08) and reviewed uninstall with retained data (F09) are implemented. Archive garbage collection and permanent data deletion require their own decisions. Broader F17 reconciliation remains distinct from source eligibility.
4. **Improve navigation and complete bounded integration.** Shared collection browsing (F02), quick search (F03), concise activity (F04), map quick edits (F05), and add-on settings disclosures (F10) are implemented. Further bounded integrations can be scheduled independently as their benefit warrants; the groups express priorities and dependencies, not a requirement to finish every earlier item first.
