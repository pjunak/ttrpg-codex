...
rewrite is not a production replacement for v1. The former generic campaign
shell was removed because it obscured the missing product surface. The
frontend now states that it is a development build until the old workflows are
ported, deliberately redesigned, or explicitly retired by the maintainer.
...
open or while the development-status page exists.
...
- [ ] Remove the development-status page only after every earlier gate is closed
  and the maintainer explicitly accepts v2 as the production replacement.
