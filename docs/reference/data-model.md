...
`party` palette; `campaign-shape-v1` cleans up legacy rows at server startup.
...
payload is a non-array object, and merges browser defaults. Persistent schema
migrations run under the server write lock before the server starts listening;
`load()` never rewrites fetched campaign data. On an empty server, `load()` keeps defaults
...
Ordinary JSON and multipart calls use `web/js/api-client.js` for consistent
same-origin credentials, response parsing, structured errors, and
`store:auth-failed` signaling. The `/api/data` load/write queue below remains
specialized because it owns retries, supersession, last-valid-state retention,
and save-failure events.
