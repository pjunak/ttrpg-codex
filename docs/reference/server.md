...
| GET | `/api/addons` | — | List installed addons (the registry's public projection), including each addon's `apiVersion`, `hostVersion`, capability declaration, dependencies, load state, and deterministic `contentRevision`. The revision hashes the active package hash/version plus normalized, sorted disabled content-group state; unchanged state is stable, while an update/rollback or group toggle changes it. The client re-applies the same strict compatibility decision used at install; an invalid addon is blocked without preventing unrelated addons from loading. Readable by any caller because client boot loads addons before login; never exposes the allowlist, repo URL, or credentials. For the **real DM only** the payload additionally describes whether a GitHub token is configured, never its value. See **Addon framework**. |
