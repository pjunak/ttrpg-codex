...
- **`host` facade (permission-scoped)**: built from the addon's GRANTED
  ...
  **`store.patchAddonData(collection,id,fn)`**
  ...
  (`esc`/`dataAction`/`dataOn`/`renderMarkdown`/`markdownTextarea`/`slugify`/`breadcrumb`/`icon` — addons
  ...
  addon stat tiles don't ship their own SVGs; `markdownTextarea(id,value,opts)`
  emits the stable host-upgradeable Markdown field with rich-format dropdowns,
  live textarea sync, preview, and draft recovery), **`host.asset(rel)`** (always available — URL of a
