...
type Commit struct {
	...
	CollectionRevisions map[campaign.Collection]int64 `json:"collectionRevisions"`
}
...
func (snapshot Snapshot) LegacyDataset(
...
) campaign.LegacyDataset {
	dataset := campaign.LegacyDataset{
		Present:     make([]campaign.Collection, 0),
		Records:     make([]campaign.LegacyRecord, 0, len(snapshot.Records)),
	...
	for _, record := range snapshot.Records {
		dataset.Records = append(dataset.Records, campaign.LegacyRecord{
